...
class HabitatComponent_Stateviewer_Entity extends LitElement {
  ...
  updateState(_pathDetail, _entity, _entityState) {
    ...
    this.entity = _entity;
    if (this.isRendered) this.renderState(_pathDetail, _entityState);
  }

  renderState(_pathDetail, _entityState) {
    if (_pathDetail && typeof _pathDetail.value === 'object') this.renderObject(_pathDetail.path, _pathDetail.value);else if (_pathDetail) this.renderPath(_pathDetail.path, _pathDetail.value);else this.renderObject('', _entityState);
  }
  ...
  renderObject(_path = '', _object) {
    for (var property in _object) {
      if (_object.hasOwnProperty(property)) {
        ...
        if (typeof _object[property] === 'object') this.renderObject(curPath, _object[property]);else this.renderPath(curPath, _object[property]);
      }
    }
  }
  ...
  renderPath(_path, _value) {
    ...
    let newTextElement;
    let signalElement;
    if (pathValueElement) pathValueElement.innerHTML = _value.toString();else {
    ...
    signalElement = pathValueElement ? pathValueElement : newCellElement; // do some animation of the background so the user does see what has changed
